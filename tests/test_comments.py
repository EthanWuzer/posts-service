...
def sample_post_doc(sample_post_id, sample_comment_doc):
    return {
        ...
        "username": "alice",
        "userProfilePictureUrl": "https://example.com/alice.jpg",
        "imgUrl": "https://r2.example.com/img.jpg",
        "caption": "Hello world",
        "timestamp": "2026-04-07T00:00:00+00:00",
        "likedBy": [],
...
async def test_add_comment_bad_input(sample_post_id):
    ...
    try:
        async with AsyncClient(
            ...
            response = await client.post(
                f"/posts/{sample_post_id}/comments",
                json={"text": "Missing username and userProfilePictureUrl"},
            )
    finally:
        app.dependency_overrides.clear()
...
@pytest.mark.asyncio
async def test_add_comment_no_auth(sample_post_id, sample_comment_body):
    app.dependency_overrides[get_db] = lambda: make_mock_collection()
    # No auth override
...
@pytest.mark.asyncio
async def test_delete_comment_forbidden(sample_post_id, sample_post_doc, sample_comment_id):
    mock_collection = make_mock_collection(
        find_one=AsyncMock(return_value=sample_post_doc),
    )
    app.dependency_overrides[get_db] = lambda: mock_collection
...
@pytest.mark.asyncio
async def test_delete_comment_not_found_comment(sample_post_id, sample_post_doc):
    mock_collection = make_mock_collection(
        find_one=AsyncMock(return_value=sample_post_doc),
    )
    app.dependency_overrides[get_db] = lambda: mock_collection
